...
Версия документа: 2026-07-30. Это целевой контракт всей переработки. В Stage 2 реализован только согласованный срез товаров, приходов и продаж: `/api/products`, `/api/brands`, `/api/categories`, `/api/receipts`, `/api/sales` и их вложенные ресурсы. Для параллельной разработки каждый реализованный маршрут также имеет alias `/api/v1/...`. Остальные разделы остаются проектом и не считаются реализованными.

Фактические уточнения Stage 2:

- массовое изменение товаров: `PATCH /api/products/bulk`;
- каталоги форм: `GET /api/receipts/catalog` и `GET /api/sales/catalog`;
- справочники продаж: `GET /api/sales/sources` и `GET /api/sales/locations`;
- unsafe-запросы авторизованной сессии принимают CSRF только через `X-CSRF-Token`;
- ошибки Stage 2 имеют стабильные `code`, `message`, `request_id` и необязательный `fields`;
- текущие Jinja-маршруты и их form handlers не удалены и не переключены на React.
