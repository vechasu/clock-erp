...
## 6. Дополнительный production-дефект редактирования

После первого исправления production-проверка обнаружила исторический приход,
у которого были сохранены только старый внешний `product_id` и текстовые
снимки бренда/категории. Общих `brand_id` и `category_id` у записи не было,
поэтому React-форма не могла восстановить выбранные значения.

Миграция `unified_catalog_v2_legacy_products` переносит такие однозначные
исторические снимки в общий каталог с нулевым начальным остатком, сохраняет
внешний ID как связь, выдаёт общие числовые ID бренда и категории и связывает
исторические продажи/приходы с одной карточкой. Суммарный остаток при миграции
не меняется; конфликтующие или неполные снимки остаются в отчёте для ручной
проверки.
