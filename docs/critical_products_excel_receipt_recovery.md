...
3. Заголовки определяются по названиям. Числа, Excel serial, даты, время и
   формулы блокируются как названия товара. Нулевые строки явно исключаются с
   объяснением.
4. Предпросмотр показывает исходную строку, позицию, количество, результат
   ...
   не меняются.
5. Только отдельный POST «Оформить приход» повторно читает сохранённый файл и в
   одной SQLite-транзакции создаёт документ, строки, недостающие карточки и
   отдельные операции остатков.
6. Уникальные SHA-256 и `draft_id` делают повторную загрузку и повторное
   подтверждение идемпотентными. Исключение на любой строке откатывает документ,
   карточки и операции целиком.
