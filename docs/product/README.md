...
| Ремонты | `current`: полный цикл ремонтного обращения | `/app/repairs`; создание, редактирование, статусы, действия, архив/восстановление, логистика и вложения; JSON API `/api/v1/repairs` | `app/web.py`, `app/services/repair_cases.py`, `app/templates/repair.html` | `tests/test_repairs_full_cycle.py`, `tests/test_stage2_repairs_api.py`, `tests/test_legacy_repair_import.py` | Хранилище включает JSON и файлы вложений; импорт legacy-данных является отдельной операцией |
