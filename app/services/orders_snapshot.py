    ...
    def update_wildberries_statuses(self, statuses):
        ...
        with self.connection() as connection:
            ...
            for external_id, status in statuses.items():
                ...
                connection.execute(
                    "UPDATE orders_snapshot SET status=?,payload_json=?,loaded_at=? WHERE order_id=?",
                    (after[0], json.dumps(order, ensure_ascii=False), datetime.now().timestamp(), row['order_id']))
        return changed
