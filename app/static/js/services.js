(function () {
    ...
    if (dialog) {
        ...
        form.addEventListener("submit",async function(event){event.preventDefault();var status=document.getElementById("serviceFormStatus");status.textContent="";
            ...
            try{await api(id?"/api/services/"+id:"/api/services",{method:id?"PUT":"POST",body:body});dialog.close();notify(id?"Изменения сохранены":"Сервис добавлен");await refresh();}catch(error){status.textContent=error.message;}
        });
    }
