"""Deploy-time catalog schema source of truth.

Runtime code must never import or execute this module. It is owned exclusively
by the versioned catalog migration runner.
"""
...
def apply_fresh_catalog_schema(connection, ddl_observer=None):
    ...
    _apply_product_deletion_columns(connection)
    _apply_product_workflow_columns(connection)
    _apply_product_image_columns(connection)
    _apply_receipt_constraints(connection)
    _apply_optional_price_constraints(connection)
    _apply_shared_catalog(connection)
    _apply_brand_image_columns(connection)
    _apply_brand_category_relations(connection)
...
def migration_source_checksum():
    ...
    steps = [
        ...
        _apply_product_deletion_columns,
        _apply_product_workflow_columns,
        _apply_product_image_columns,
        _apply_receipt_constraints,
        _apply_optional_price_constraints,
        _apply_shared_catalog,
        _apply_brand_image_columns,
        _apply_brand_category_relations,
        _apply_inventory_constraints,
        _apply_stock_movement_constraints,
        apply_order_comment_constraints,
        _apply_order_item_mapping_schema,
    ]
    source = [CATALOG_SCHEMA_SQL]
    source.extend(inspect.getsource(step) for step in steps)
...
def _apply_excel_receipt_constraints(connection):
...
def _apply_excel_cardinality_columns(connection):
    migrations = {
...
def _apply_excel_import_draft_schema(connection):
...
def _apply_product_deletion_columns(connection):
...
def _apply_product_workflow_columns(connection):
...
def _apply_product_image_columns(connection):
...
def _apply_receipt_constraints(connection):
...
def _apply_optional_price_constraints(connection):
...
def _apply_shared_catalog(connection):
    ...
    def get_or_create_category(brand_id, name):
        display_name = cleaned(name)
        key = normalized(display_name)
    ...
    for product in products:
        brand = get_or_create_brand(product["excel_brand"])
...
def _apply_brand_image_columns(connection):
...
def _apply_brand_category_relations(connection):
...
def _apply_inventory_constraints(connection):
...
def _apply_stock_movement_constraints(connection):
...
def apply_order_comment_constraints(connection):
    """Rebuild known legacy comment tables to the complete catalog contract."""
    comment_sql_row = connection.execute(
        "SELECT sql FROM sqlite_master WHERE type = 'table' "
        "AND name = 'erp_order_comments'"
    ).fetchone()
    sync_foreign_keys = connection.execute(
        "PRAGMA foreign_key_list(erp_order_comment_sync_state)"
    ).fetchall()
    normalized_sql = "".join(str(comment_sql_row[0] or "").lower().split())
    has_text_check = "check(length(trim(text))>0)" in normalized_sql
    has_comment_reference = any(
        str(row[2]) == "erp_order_comments"
        and str(row[3]) == "last_outbound_comment_id"
        and str(row[4]) == "id"
        and str(row[6]).upper() == "SET NULL"
        for row in sync_foreign_keys
    )
    if has_text_check and has_comment_reference:
        return
    blank = connection.execute(
        "SELECT id FROM erp_order_comments "
        "WHERE text IS NULL OR length(trim(text)) = 0 LIMIT 1"
    ).fetchone()
    if blank is not None:
        raise RuntimeError(
            "Order comment migration found an empty legacy comment"
        )
    ...
    try:
        ...
        connection.executescript("""
            DROP TABLE IF EXISTS erp_order_comment_sync_state_values;
            DROP TABLE IF EXISTS erp_order_comments_migrating;
            CREATE TABLE erp_order_comments_migrating (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                order_id TEXT NOT NULL,
                text TEXT NOT NULL CHECK (length(trim(text)) > 0),
                author_name TEXT NOT NULL,
            ...
            CREATE TABLE erp_order_comment_sync_state_values (
                order_id TEXT PRIMARY KEY,
                external_order_id TEXT NOT NULL,
                last_external_hash TEXT,
                last_external_updated_at TEXT,
                last_outbound_comment_id INTEGER,
                updated_at TEXT NOT NULL
            );
        """)
        comment_columns = (
            "id, order_id, text, author_name, author_user_id, created_at, "
            "updated_at, external_system, external_id, external_updated_at, "
            "sync_status, sync_hash, source, sync_attempts, next_retry_at, "
            "last_sync_error"
        )
        connection.execute(
            "INSERT INTO erp_order_comments_migrating ({}) "
            "SELECT {} FROM erp_order_comments".format(
        ...
        connection.execute(
            "INSERT INTO erp_order_comment_sync_state_values ({}) "
            "SELECT {} FROM erp_order_comment_sync_state".format(
                sync_columns, sync_columns
            )
        )
        connection.executescript("""
            DROP TABLE erp_order_comment_sync_state;
            DROP TABLE erp_order_comments;
            ALTER TABLE erp_order_comments_migrating
                RENAME TO erp_order_comments;
            CREATE TABLE erp_order_comment_sync_state (
                ...
                last_external_updated_at TEXT,
                last_outbound_comment_id INTEGER
                    REFERENCES erp_order_comments(id) ON DELETE SET NULL,
                updated_at TEXT NOT NULL
            );
            ...
            SELECT order_id, external_order_id, last_external_hash,
                last_external_updated_at, last_outbound_comment_id, updated_at
            FROM erp_order_comment_sync_state_values;
            DROP TABLE erp_order_comment_sync_state_values;
            CREATE INDEX idx_erp_order_comments_order_created
                ON erp_order_comments(order_id, created_at, id);
        """)
        connection.commit()
    except Exception:
    ...
    if connection.execute("PRAGMA foreign_key_check").fetchall():
        raise RuntimeError(
            "Order comment migration created foreign key violations"
        )


def _apply_order_item_mapping_schema(connection):
    ...
    try:
        ...
        """)
        connection.execute(
            "INSERT INTO erp_order_product_mappings_migrating "
