...
test('orders compact header and list remain stable at target widths', async ({ page }) => {
  ...
  for (const width of [1920, 1440, 1024, 768, 390]) {
    ...
    const headers = await page.locator('.orders-table th').allTextContents();
    expect(headers).toEqual([
      'Заказ', 'Создан', 'Статус', 'Сумма', 'Покупатель',
      'Оплата', 'Товары', 'Доставка', 'Комментарии',
    ]);
    const structure = await page.locator('.orders-table').evaluate((table) => table.innerHTML);
    await page.waitForTimeout(1200);
    await expect(page.locator('.orders-table')).toHaveJSProperty('innerHTML', structure);
  }
...
test('dense rows show all products, separate comments, and neutral missing values', async ({ page }) => {
  await page.goto('/app/orders', { waitUntil: 'networkidle' });
  const first = page.locator('tr[data-order-id="7001"]');
  ...
  await expect(page.locator('tr[data-order-id="7003"]')).toContainText('—');
});
